...
class Student:
    ...
    def __init__(
        self, student_id: int, name: str, gpa: float, skills: List[str] = None
    ) -> None:
        self.id = student_id
        self.name = name
        self.gpa = gpa
    ...
    def check_profile_completion(self) -> str:
        ...
        if not self.skills:
            ...
            return "Профиль заполнен частично (рекомендуется добавить минимум 3 навыка)."
    ...
    def calculate_rating(self, projects_count: int) -> float:
        ...
        # Формула: (GPA * 10) + (количество проектов * 15)
